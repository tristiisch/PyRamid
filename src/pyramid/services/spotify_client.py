...
class SpotifyClientService(Spotify, ISpotifyClientService, ServiceInjector):
	...
	async def _get_async(self, url, args=None, payload=None, **kwargs) -> dict[str, Any]:
		if args:
			kwargs.update(args)

		return await self._async_internal_call("GET", url, payload, kwargs)

	async def _async_internal_call(self, method: str, url: str, payload, params) -> dict[str, Any]:
